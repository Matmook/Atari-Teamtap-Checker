...
rasters_enable:
    ...
    move.l  #timerb_isr_1,$120      ; MFP's Timer B new vector
...
rasters_disable:
    ...
    ; TimerB isr 1
    ; will set the second palette and setup Timer B
    .long
timerb_isr_1:    
    clr.b   $fffffa1b.w             ; disable Timer B
    move.w  pal_background,$ff8240  ; set background now to avoid uncomplete background line!

    move.l  #timerb_isr_2,$120      ; MFP's Timer B new vector
    move.b  #(160-13),$fffffa21     ; set Timber B offset (change at another line!)
    ...
    ; TimerB isr 2
    ; will set the third palette
    .long
timerb_isr_2:
    clr.b   $fffffa1b.w             ; timer stop
    move.w  pal_jagware,$ff8240     ; set background now to avoid uncomplete background line!

    ; bottom screen part
    movem.l  d0-d7,-(sp)            ; save context
    movem.l  pal_jagware, d0-d7     ; put picture palette in d0-d7
    movem.l  d0-d7, $ff8240         ; move palette from d0-d7
    movem.l  (sp)+,d0-d7            ; restore context
    ...
    ; VBL isr
    ; will set the first palette and setup Timer B
    .long
new_vbl_isr:    
    ...
    ; 1st screen part
    move.w  pal_matmook,$ff8240     ; set background now to avoid uncomplete background line!
    movem.l  d0-d7,-(sp)            ; save context
